...
const rankTableTotal = document.getElementById("rankTableTotal");
...
function renderMemberList() {
  ...
  state.members.forEach((member) => {
    const salary = getRankSalary(member.rank);
    ...
    card.innerHTML = `
      <div class="member-info">
        ...
        <p class="member-salary">${formatCredits(salary)}</p>
      </div>
...
/* ----------------------------------------------------------
   9. RENDER: RANK SALARY TABLE (with live TỔNG)
   ---------------------------------------------------------- */
function renderRankTable() {
  ...
  RANK_IDS.forEach((rankId) => {
    ...
    row.innerHTML = `
      <td>${rankId}</td>
      <td>${getRankLabel(rankId)}</td>
      <td>${formatShort(getRankSalary(rankId))}</td>
    `;
    rankTableBody.appendChild(row);
  });
  const totalActual = calcTotalSalary();
  rankTableTotal.textContent = formatCredits(totalActual);
}
...
function calcTotalSalary() {
  return state.members.reduce((sum, m) => sum + getRankSalary(m.rank), 0);
}
